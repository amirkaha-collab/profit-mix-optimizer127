...
def render_product_selector():
    ...
    # ── Row 1: optimizer buttons LEFT, קרנות השתלמות RIGHT ───────────────
    ...
    # ── c1: "ניתוח תיק לקוח" — special highlighted tab ─────────────────
    with nav_cols[0]:
        is_client = (pt == "תיק לקוח")
        client_css = (
            "background:linear-gradient(135deg,#0f4c81,#1a6bb5);color:#fff;"
            "font-weight:900;font-size:14px;border:none;border-radius:10px;"
            "padding:9px 0;width:100%;cursor:pointer;box-shadow:0 2px 8px rgba(15,76,129,.35);"
            if is_client else
            "background:linear-gradient(135deg,#e8f0fe,#c7d9f8);color:#0f4c81;"
            "font-weight:800;font-size:14px;border:2px solid #4a90d9;"
            "border-radius:10px;padding:8px 0;width:100%;cursor:pointer;"
        )
        st.markdown(
            f"<div style='{client_css}text-align:center'>📊 ניתוח תיק לקוח</div>",
            unsafe_allow_html=True,
        )
        if st.button("📊 ניתוח תיק לקוח", type="primary" if is_client else "secondary",
                     use_container_width=True, key="pt_client",
                     help="ניתוח תיק לקוח מפורט"):
            if not is_client:
    ...
    # ── c2–c5: standard optimizer tabs ──────────────────────────────────
    for (_, label, val, key), col in zip(_optimizer_btns, nav_cols[1:5]):
        with col:
    ...
    with nav_cols[5]:
        st.markdown(
            "<div style='height:38px;border-right:2px solid #D1D5DB;"
            "margin:0 auto;width:1px'></div>",
            unsafe_allow_html=True,
        )

    # ── קרנות השתלמות — prominent RIGHT button (no duplicate div) ────────
    with nav_cols[6]:
        is_funds = (pt == "קרנות השתלמות")
        if st.button("🏦 קרנות השתלמות",
                     type="primary" if is_funds else "secondary",
    ...
    # ── Row 2: "ניתוח אסטרטגיות מוסדיים" tab — below ניתוח תיק לקוח ────
    is_isa = (pt == "אסטרטגיות מוסדיים")
    isa_cols = st.columns([1.2, 5.55])
    with isa_cols[0]:
        isa_css = (
            "background:linear-gradient(135deg,#134e3a,#16a34a);color:#fff;"
            "font-weight:800;font-size:12px;border:none;border-radius:8px;"
            "padding:5px 0;width:100%;cursor:pointer;box-shadow:0 2px 6px rgba(22,163,74,.3);"
            if is_isa else
            "background:#f0fdf4;color:#14532d;font-weight:700;font-size:12px;"
            "border:1.5px solid #86efac;border-radius:8px;"
            "padding:5px 0;width:100%;cursor:pointer;"
        )
        st.markdown(
            f"<div style='{isa_css}text-align:center'>📐 ניתוח אסטרטגיות מוסדיים</div>",
            unsafe_allow_html=True,
        )
        if st.button("📐 ניתוח אסטרטגיות מוסדיים",
                     type="primary" if is_isa else "secondary",
                     use_container_width=True, key="pt_isa",
                     help="ניתוח השוואתי של אסטרטגיות השקעה בין גופים מוסדיים"):
            if not is_isa:
                st.session_state["product_type"] = "אסטרטגיות מוסדיים"
                st.rerun()
...
# ── INSTITUTIONAL STRATEGY ANALYSIS MODE: bypass optimizer ──
if _product_type_now == "אסטרטגיות מוסדיים":
    try:
        from institutional_strategy_analysis.ui import render_institutional_analysis
        render_institutional_analysis()
    except Exception as _isa_mode_err:
        st.error(f"שגיאת טעינה: {_isa_mode_err}")
        import traceback; st.code(traceback.format_exc())
    st.stop()
...
# ── INSTITUTIONAL STRATEGY ANALYSIS is accessed via the dedicated tab ──
# (select "ניתוח אסטרטגיות מוסדיים" in the navigation above)
